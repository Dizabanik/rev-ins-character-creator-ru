...
export const generateBackstory = async (character: Character): Promise<string> => {
  ...
  const appearanceInfo = `
    Внешность и прошлое:
      ...
      Рост: ${character.height || 'Не указан'}
      Вес: ${character.weight || 'Не указан'}
      Цвет глаз: ${character.eyeColor || 'Не указан'}
      Волосы: ${character.hairColor || 'Не указан'}
      Личная предыстория (до телепортации): ${character.manualBackstory || 'Нет подробностей.'}
  `;
  ...
  const prompt = `
    ...
    Как его расовые особенности, внешность, личная предыстория, итоговые характеристики, навыки, черты, предметы, физическое состояние (HP, истощение) ИЛИ состояние его апертуры/эссенции (включая её относительную "ценность" из-за конденсации, если применимо и известно ему) влияют на его первые несколько мгновений или часов выживания или его немедленные реакции?
